...
This vignette provides a comprehensive overview of the package functionality, including:
- How to prepare input data
- Steps to generate a custom xCell2 reference object
- How to use the main functions for cell type enrichment analysis
- Interpretation of results and best practices
Whether you're new to cell type deconvolution or an experienced bioinformatician, this guide will help you harness the full potential of xCell 2.0 in your research.

# Installation

The `xCell2` package is designed to be used with R version 4.4.0 or higher. To install `xCell2`, you can use one of the following methods:

## From Bioconductor (Coming Soon)

Once `xCell2` is available on Bioconductor, you can install it using the following commands:
```{r, eval = FALSE}
...
```
Note that installing from GitHub may require additional setup, such as having development tools installed on your system.

## Dependencies

xCell2 relies on several Bioconductor packages. Most dependencies will be automatically installed. However, if you encounter any issues, you may need to manually install some packages, particularly ontoProc (version 1.26.4 or higher).
...
1. A reference gene expression matrix
2. A labels data frame
...
1. "ont": Cell type ontology (e.g., "CL:0000545" or NA if not applicable)
2. "label": Cell type name (e.g., "T-helper 1 cell")
3. "sample": Sample/cell identifier matching column names in the reference matrix
4. "dataset": Source dataset or subject identifier
...
First, save your newly generated xCell2 reference object:
```{r, eval = FALSE}
save(DICE_demo.xCell2Ref, file = "DICE_demo.xCell2Ref.rda")
```

2. Prepare Your Reference for Sharing
Before sharing, ensure your reference includes:
...
After adding your reference and updating the README:
...
Remember, creating a robust reference is crucial for accurate results. Take time to ensure your input data is high-quality and properly annotated.
...
You can download a pre-trained reference directly within R using the `download.file()` function. Here's an example using the BlueprintEncode reference:
```{r, eval = FALSE}
...
Remember to choose a reference that's appropriate for your specific tissue type and experimental context. The choice of reference can significantly impact your results, so it's important to select one that closely matches your biological system.
...
Once you have your xCell 2.0 results, you can:
...
Remember, xCell 2.0 provides estimates of relative cell type abundance. For absolute quantification, additional experimental validation may be necessary.

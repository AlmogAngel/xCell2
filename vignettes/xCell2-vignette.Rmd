...
*seed* Set seed for reproducible results (default: NULL).
...
```{r, eval = TRUE}
save(DICE.xCell2Ref, file = "DICE.xCell2Ref.rda")
```
...
In this example we will load the human PanCancer reference made from scRNA-Seq data.
```{r, eval = TRUE}
?xCell2::PanCancer.xCell2Ref
PanCancer.xCell2Ref <- xCell2::PanCancer.xCell2Ref
```

Now you can run `xCell2Analysis`:
```{r, eval = TRUE}
xcell2_res <- xCell2::xCell2Analysis(mix, xcell2object = PanCancer.xCell2Ref)
```

...
#   blood_celltypes <- c(blood_celltypes, rownames(file.in))
# }
#
...
#
# celltypes2use <- names(which(table(blood_cts) > 1))
...
#
#
# sref.blood <- sref.data[,sref.labels.blood$sample]
# rm(sref.data)
#
# ref <- sref.blood
# labels <- as.data.frame(sref.labels.blood)
#
# saveRDS(sref.blood, "/bigdata/almogangel/xCell2/dev_data/sref_blood_data_bulk.rds")

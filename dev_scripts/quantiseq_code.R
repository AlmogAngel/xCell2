# Credit: https://github.com/icbi-lab/quanTIseq/tree/master/quantiseq/deconvolution
...
mapGenes<-function(mydata) {
  ...
  HGNC<-read.csv("/opt/quantiseq/deconvolution/HGNC_genenames_20170418.txt", header=TRUE, sep="\t")
...
celldensities <- function(DCres){
  ...
  imageinfo <- read.csv("/opt/quantiseq/deconvolution/totalcells.txt", header=FALSE, sep="\t", stringsAsFactors = FALSE)

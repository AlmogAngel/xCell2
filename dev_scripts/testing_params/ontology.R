...
res_noontology <- mclapply(1:nrow(vals.refs.res), function(i) {
...
res_ontology <- mclapply(1:nrow(vals.refs.res), function(i) {
...
results <- list(res_noontology, res_ontology)

# Define the corresponding method descriptions
method_descriptions <- c("No Ontology", "Ontology")
...
saveRDS(allcors, "/bigdata/almogangel/xCell2/dev_scripts/testing_params/ontology_allcors_cyto.rds")
...
# weight_genes <- TRUE
# res_weights.sc <- mclapply(1:nrow(vals.refs.res.sc), function(i) {
#
#   # Load data
...
#
# weight_genes <- FALSE
# res_noweights.sc <- mclapply(1:nrow(vals.refs.res.sc), function(i) {
#
#   # Load data
#   val_ref <- paste0(vals.refs.res.sc[i,]$val_dataset, "_", vals.refs.res.sc[i,]$ref_name[[1]])
#   print(val_ref)
...
#
# # Define the vector of list names
# results <- list(res_weights.sc, res_noweights.sc)
#
# # Define the corresponding method descriptions
# method_descriptions <- c("Weights", "No Weights")
#
#
# allcors.sc <- purrr::map2(results, method_descriptions, ~ {
#   vals.refs.res.sc %>%
#     ungroup() %>%
#     select(method, val_type, val_dataset, n_val_samples) %>%
#     mutate(method = .y,
...
#
#
# saveRDS(allcors.sc, "/bigdata/almogangel/xCell2/dev_scripts/testing_params/genes_weights_allcors_sc.rds")
#
#
# # allcors <- allcors %>%
# #   group_by(method) %>%
